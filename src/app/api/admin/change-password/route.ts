...
export async function POST(request: NextRequest) {
  try {
    // Rate limiting to prevent brute force attacks on current password
    const ip = getClientIp(request)
    const rateCheck = checkRateLimit(`admin_change_password:${ip}`, { windowMs: 60 * 60 * 1000, maxRequests: 5 })
    if (!rateCheck.allowed) {
      return NextResponse.json(
        { success: false, error: 'ಹೆಚ್ಚು ಪ್ರಯತ್ನಗಳು. ದಯವಿಟ್ಟು 1 ಗಂಟೆಯ ನಂತರ ಪ್ರಯತ್ನಿಸಿ.' },
        { status: 429 }
      )
    }
