...
const updateProfileSchema = z.object({
  ...
  phone: z.union([z.string().regex(/^\d{10}$/, 'ದೂರವಾಣಿ ಸಂಖ್ಯೆ 10 ಅಂಕೆಗಳಿರಬೇಕು'), z.literal('')]).optional()
  ...
  state: z.string().max(100).optional()
    .transform(v => (v === undefined ? undefined : (v ? sanitize(v) : null))),
  pincode: z.union([z.string().regex(/^\d{6}$/, 'ಪಿನ್‌ಕೋಡ್ 6 ಅಂಕೆಗಳಿರಬೇಕು'), z.literal('')]).optional()
    .transform(v => (v === undefined ? undefined : (v ? sanitize(v) : null)))
})
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!rateCheck.allowed) {
      return NextResponse.json(
        { success: false, error: 'ಹೆಚ್ಚು ಪ್ರಯತ್ನಗಳು. ದಯವಿಟ್ಟು ಸ್ವಲ್ಪ ಸಮಯದ ನಂತರ ಪ್ರಯತ್ನಿಸಿ.' },
        { status: 429 }
      )
    }
    ...
    if (!result.success) {
      const errorMessage = result.error.issues[0]?.message || 'ಅಮಾನ್ಯ ಡೇಟಾ'
      return NextResponse.json(
