...
## 2025-03-27 - Critical: Missing Authorization in Destructive Admin APIs
**Vulnerability:** Several mutative API endpoints (`PATCH` and `DELETE` on `/api/categories/[id]` and `/api/books/[slug]`, and `PUT` on `/api/settings/hero`) lacked explicit `verifyAdminSession` authorization checks in their route handlers. They relied completely on Next.js path-based middleware for protection, which violates defense-in-depth principles.
**Learning:** Destructive operations (modifying or deleting categories, books, or site settings) must independently verify authorization within the route handler. Relying solely on path-based middleware creates a single point of failure and makes the application fragile to routing changes or middleware misconfigurations.
**Prevention:** Enforce a strict policy where *every* mutative API route handler explicitly invokes an authorization verification function (e.g., `verifyAdminSession`) at the very beginning of its execution, regardless of the overarching middleware.
