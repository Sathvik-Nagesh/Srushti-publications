...
## 2025-05-31 - [Insecure PRNG for Session IDs]
**Vulnerability:** The `/api/wishlist/route.ts` backend API endpoint used `Math.random().toString(36).substr(2, 9)` combined with `Date.now()` to generate session IDs. `Math.random()` is not a cryptographically secure pseudo-random number generator (CSPRNG), which makes the generated session IDs predictable and susceptible to session hijacking or predictable token vulnerabilities.
**Learning:** `Math.random()` should never be used for generating secure tokens, session IDs, or any security-critical identifiers, especially in backend environments where `crypto` is readily available. In Next.js App Router edge/node environments, `crypto.randomUUID()` provides a robust and secure standard for generating UUID v4 identifiers. Truncation of tokens should also be avoided to maintain entropy.
**Prevention:** Always use `crypto.randomUUID()` or `crypto.getRandomValues()` for generating secure identifiers on the backend. When using UUIDs, do not truncate them.
