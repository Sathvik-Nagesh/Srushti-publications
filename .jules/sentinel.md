...
## 2025-03-10 - Critical: Missing Authorization on Non-Admin Modification Endpoints
**Vulnerability:** The root-level API routes for books (`/api/books/[slug]`) and categories (`/api/categories/[id]`) exposed `PATCH` and `DELETE` methods that lacked explicit authorization checks. Because these routes do not fall under the `/admin` or `/api/admin` paths, they completely bypassed the path-based middleware protection, allowing unauthenticated users to modify or delete books and categories.
**Learning:** Destructive operations (PATCH, PUT, DELETE) on core resources should always be protected at the route handler level using explicit authorization checks (e.g., `verifyAdminSession`). Relying on middleware for partial sub-paths is dangerous because API designs often place resource modification at the root level (RESTful design), outside of protected sub-paths.
**Prevention:** Enforce a strict policy that *every* data modification endpoint (POST, PUT, PATCH, DELETE) that performs administrative actions must explicitly verify admin sessions internally, independent of the routing hierarchy or middleware coverage.
