...
## $(date +%Y-%m-%d) - Prevent Stored XSS and Add Rate Limiting to Profile Update (`POST /api/auth/me`)
...
## 2024-06-20 - Insecure Session ID Generation in Wishlist API
**Vulnerability:** The wishlist API (`src/app/api/wishlist/route.ts`) was using `Math.random().toString(36)` to generate session identifiers. This is a predictable, non-cryptographically secure Pseudo-Random Number Generator (PRNG), making session IDs susceptible to guessing/brute-forcing, which could lead to Session Hijacking or Insecure Direct Object Reference (IDOR) vulnerabilities where an attacker could manipulate other users' wishlists.
**Learning:** `Math.random()` should never be used for security-sensitive operations like generating tokens or session identifiers. Even if combined with timestamps (`Date.now()`), the entropy is insufficient against determined attackers.
**Prevention:** Always use a Cryptographically Secure Pseudo-Random Number Generator (CSPRNG) for tokens and session IDs. In Next.js backend routes (Node.js/Edge runtime), `crypto.randomUUID()` or `crypto.getRandomValues()` from the Web Crypto API are the correct tools for generating secure, unpredictable identifiers.
