...
## $(date +%Y-%m-%d) - Critical: Predictable Session ID Generation in Wishlist API
**Vulnerability:** The `src/app/api/wishlist/route.ts` endpoint generated new wishlist session IDs using `Math.random()` and `Date.now()`. `Math.random()` is not a cryptographically secure pseudo-random number generator (CSPRNG), making the session tokens predictable. An attacker could potentially guess session IDs, leading to session hijacking and Insecure Direct Object Reference (IDOR) vulnerabilities where they could modify or view other users' wishlists.
**Learning:** Never use `Math.random()` for security-sensitive tokens, identifiers, or session IDs. Predictability undermines the randomness required to keep sessions private and secure.
**Prevention:** Always use the Web Crypto API's `crypto.randomUUID()` or `crypto.getRandomValues()` to generate cryptographically secure identifiers and tokens in backend routes. Note: Ensure usage is within backend/API contexts, as `window.crypto` is unavailable in non-secure HTTP environments on the client-side.
