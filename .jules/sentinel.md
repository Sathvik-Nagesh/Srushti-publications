...
## 2025-03-04 - High: Missing Authorization in Admin Settings
**Vulnerability:** The `/api/admin/settings` endpoint was missing the `verifyAdminSession` authorization check in both `GET` and `PATCH` methods. While middleware attempts to protect `/api/admin/*` paths, relying exclusively on middleware for authorization is an anti-pattern. If the middleware is misconfigured or bypassed, the endpoint is fully exposed, allowing unauthorized users to view or modify global site settings.
**Learning:** Admin endpoints must explicitly enforce authorization checks within their route handlers. Relying solely on path-based middleware protection is brittle and prone to bypasses.
**Prevention:** Always enforce `verifyAdminSession(request)` at the beginning of every admin API route handler, ensuring defense-in-depth regardless of middleware coverage.
