...
## 2026-03-07 - Medium: Information Disclosure via API Error Details\n**Vulnerability:** The `/api/reviews` POST endpoint returned the raw error message to the client in the `details` field of a 500 JSON response upon failure. This could potentially leak internal database structures, code logic, or sensitive configuration details to potential attackers.\n**Learning:** API route handlers should never expose raw `error.message` or `error.stack` details directly to the client. This is a common Information Disclosure vulnerability.\n**Prevention:** Catch blocks should log the full error details server-side using `console.error`, but return generic, localized error messages to the client (e.g., 'An error occurred. Please try again.'). Utilizing a centralized error handling utility like `getSafeErrorMessage` or ensuring `details` properties are omitted from API error payloads is crucial.
