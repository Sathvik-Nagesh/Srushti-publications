...
## 2025-03-12 - Critical: Missing Defense-in-Depth in Site Settings and Password Change APIs
**Vulnerability:** The endpoints `PUT /api/settings/hero` and `POST /api/admin/change-password` lacked explicit `verifyAdminSession` authorization checks in their route handlers. They relied on Next.js path-based middleware for protection. If the middleware failed or was bypassed, unauthenticated users could alter global site settings or change the admin password (if the latter fell back to environment variable verification).
**Learning:** Route-level explicit authorization checks act as the primary defense mechanism against middleware configuration errors, bypasses, or accidental public exposure of administrative routes. Relying solely on path-based middleware creates a single point of failure.
**Prevention:** Enforce a strict policy where *every* administrative or restricted API route handler explicitly invokes an authorization verification function (e.g., `verifyAdminSession`) at the very beginning of its execution, regardless of the overarching middleware.
