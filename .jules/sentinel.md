...
## 2024-05-24 - Weak Randomness in Wishlist Session ID
**Vulnerability:** The `src/app/api/wishlist/route.ts` endpoint used `Math.random()` to generate session IDs (`sessionId`). `Math.random()` is not cryptographically secure and can generate predictable tokens, making the application vulnerable to session hijacking or token guessing attacks.
**Learning:** While `Math.random()` might be used defensively in non-secure HTTP client-side contexts where `window.crypto` is unavailable, backend API routes in Next.js have full access to the Node.js Web Crypto API. The developer likely copy-pasted a client-side pattern into a server-side route.
**Prevention:** Always use `crypto.randomUUID()` or `crypto.getRandomValues()` for generating secure tokens or session IDs on the backend. Ensure the full generated UUID is preserved to maintain high entropy.
