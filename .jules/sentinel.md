...
## 2025-03-20 - High: Missing Rate Limiting on Logout Endpoints
**Vulnerability:** The customer and admin logout endpoints (`/api/auth/logout` and `/api/admin/logout`) lacked rate limiting. This presented a potential vector for Denial-of-Service (DoS) and resource exhaustion attacks by allowing malicious actors to rapidly spam session clearing requests without restriction.
**Learning:** Even though logout endpoints do not require active authorization (to allow expired sessions to clear cookies) and do not usually hit the database directly, they can still consume server resources. Every exposed endpoint that alters state or clears headers is a potential target for abuse.
**Prevention:** Apply rate limiting to all public-facing endpoints using `checkRateLimit` (e.g., 5 requests per minute per IP), including logout and session-clearing functionalities, to protect against rapid abuse.
