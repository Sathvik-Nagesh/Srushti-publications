...
## 2025-03-18 - High: Stored XSS & Missing Rate Limiting in Profile Updates
**Vulnerability:** The `/api/auth/me` endpoint accepted user-provided profile data (like address, city, state) and passed it directly to Prisma for updating without validation or sanitization, potentially allowing Stored XSS. It also lacked rate limiting, making it vulnerable to rapid update spamming. Additionally, the update logic used `field || undefined`, which prevented users from successfully clearing optional fields.
**Learning:** All endpoints accepting user input, even for authenticated users updating their own profiles, must validate and sanitize the input to prevent malicious payloads from being stored and later rendered. Rate limiting should also be applied to mutation endpoints. When updating Prisma models, `updateData.field = field` should be used over inline `|| undefined` to allow empty string clearing.
**Prevention:** Apply `checkRateLimit` on profile update endpoints. Define and use a strict Zod schema (with `.transform(sanitize)` or similar validation) to sanitize user input before passing it to database queries. Construct the Prisma update object conditionally.
