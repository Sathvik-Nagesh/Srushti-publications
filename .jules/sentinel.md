...
## 2024-03-11 - Add Rate Limiting to Order Tracking
**Vulnerability:** The `/api/orders/track` endpoint lacked rate limiting, making it vulnerable to order number enumeration attacks. An attacker could repeatedly guess sequential order numbers and extract customer names, item lists, and delivery addresses if they matched an active order.
**Learning:** Even low-value read endpoints can leak sensitive data or facilitate information disclosure if there's no restriction on brute-forcing identifiers that follow predictable patterns.
**Prevention:** Apply consistent rate limiting to all public-facing API routes, particularly those accepting identifiers or searching for specific records.
