...
## 2025-03-24 - High: Stored XSS & Missing Rate Limiting in Profile Update
**Vulnerability:** The `/api/auth/me` POST endpoint responsible for updating customer profiles accepted raw input (`name`, `address`, `phone`, etc.) without strict validation, sanitization, or rate-limiting. This allowed attackers to perform Stored Cross-Site Scripting (XSS) by injecting malicious scripts into their profile fields, which would then execute when viewed in the frontend. Additionally, the lack of rate limiting permitted profile update spam (DoS) attacks.
**Learning:** Profile update endpoints are prime targets for Stored XSS because the inputted data is frequently rendered across the application (e.g., in headers, dashboards, order summaries). Relying purely on frontend validation is insecure. Furthermore, improper handling of optional nullable fields during Prisma updates (using inline `|| undefined`) prevents users from clearing their data securely.
**Prevention:** Always enforce strict schema validation and sanitization on the server side using libraries like `zod` combined with a robust sanitizer. For Prisma updates involving optional nullable columns, construct the update object conditionally and ensure empty string inputs correctly map to `null` instead of being ignored via logical ORs. Lastly, secure all authenticated mutative endpoints with rate limiting to prevent abuse.
