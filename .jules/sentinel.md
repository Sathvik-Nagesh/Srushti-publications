...
## 2025-03-24 - [CRITICAL] Insecure Session ID Generation using Math.random()
**Vulnerability:** The `/api/wishlist/route.ts` API endpoint used `Math.random()` to generate the `wishlist_session` ID when a user accessed the wishlist without a pre-existing session cookie. Because `Math.random()` is a PRNG (Pseudo-Random Number Generator) and is not cryptographically secure, the resulting session IDs were predictable. This predictability could allow an attacker to guess or enumerate session IDs, leading to unauthorized access, state manipulation, or session hijacking of other users' wishlists.
**Learning:** `Math.random()` should never be used for security-sensitive operations such as generating session IDs, tokens, or cryptographic keys, as it does not provide sufficient entropy. In modern JavaScript environments (Node.js and Edge runtimes), the Web Crypto API is readily available and should be standard practice for generating random identifiers.
**Prevention:** Always use cryptographically secure methods like `crypto.randomUUID()` or `crypto.getRandomValues()` for generating sensitive identifiers in backend code or API routes. Ensure code reviews flag the use of `Math.random()` when applied to anything other than non-critical frontend logic.
