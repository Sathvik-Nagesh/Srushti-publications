...
## 2025-03-24 - High: Insecure Randomness for Wishlist Session ID Generation
**Vulnerability:** The `/api/wishlist` endpoint previously used `Math.random().toString(36).substr(2, 9)` to generate session IDs for users' wishlists. `Math.random()` is not cryptographically secure and produces predictable outputs. An attacker could potentially predict session IDs and hijack or manipulate other users' wishlists, leading to an Insecure Direct Object Reference (IDOR) via session hijacking.
**Learning:** `Math.random()` must never be used to generate secure identifiers, tokens, or session IDs. The predictability of the pseudo-random number generator (PRNG) exposes the application to enumeration and hijacking attacks.
**Prevention:** Always use a cryptographically secure pseudo-random number generator (CSPRNG), such as the Web Crypto API's `crypto.randomUUID()`, for generating secure tokens or session IDs on the server-side.
