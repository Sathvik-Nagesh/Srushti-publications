...
## 2025-03-20 - High: Stored XSS & DoS Vulnerabilities in Customer Profile Update
**Vulnerability:** The `/api/auth/me` POST endpoint accepted unvalidated and unsanitized JSON data for customer profiles, leading to potential Stored Cross-Site Scripting (XSS) if malicious scripts were entered in address or name fields. Additionally, the endpoint lacked rate limiting, exposing it to profile update spam and Denial of Service (DoS) attacks. Finally, using `field || undefined` to manage optional values prevented users from successfully clearing previously stored database fields using empty strings.
**Learning:** Endpoints that handle user-supplied data that is rendered back to users (like profiles) must strictly validate and sanitize the input to prevent XSS. Furthermore, all mutative or heavy endpoints must be rate-limited to avoid DoS. For database updates via Prisma, `field || undefined` turns empty strings into `undefined`, effectively dropping the field from the update and failing to clear it in the database.
**Prevention:** 1. Implement rate limiting (e.g., 5 requests/min) on endpoints using `checkRateLimit`. 2. Use a Zod schema with `.transform(sanitize)` to sanitize all inputs. 3. Map empty strings to `null` instead of `undefined` using Zod transforms `.transform(v => (v === undefined ? undefined : (v ? sanitize(v) : null)))` to correctly clear fields in Prisma. Build the Prisma data object dynamically to avoid overriding unspecified fields.
