...
## 2025-03-12 - High: Order Enumeration via Track Endpoint
**Vulnerability:** The `/api/orders/track` endpoint lacked rate limiting, allowing an attacker to brute-force order numbers (e.g., `ORD-YYMMDD-XXXX`) to discover valid orders and leak Customer PII (items, amount, shipping info, status).
**Learning:** Endpoints that query sensitive data based on predictable or semi-predictable identifiers must implement rate limiting to prevent enumeration attacks, even if the data returned is seemingly benign or partial.
**Prevention:** Implement strict rate limiting (e.g., 10 requests per minute per IP) on all endpoints that accept an identifier to query order status or details.
