...
## 2025-03-10 - Critical: Missing Authorization on Admin Settings PUT Endpoint
**Vulnerability:** The `PUT /api/settings/hero` endpoint was intended to be "Admin only" but completely lacked any authorization checks in its route handler. This Insecure Direct Object Reference (IDOR) / authorization bypass allowed any unauthenticated user to modify the site's hero settings (tagline, description, stats) by sending a direct PUT request.
**Learning:** Comments indicating an endpoint is "Admin only" do not enforce security. Without explicit route-level authorization checks, endpoints that modify global application state are exposed to everyone.
**Prevention:** Always implement explicit authorization checks (e.g., `verifyAdminSession(request)`) at the beginning of route handlers that perform administrative actions, especially those that mutate global settings or sensitive data.
