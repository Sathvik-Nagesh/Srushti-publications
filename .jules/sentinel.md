...
## 2024-05-24 - [Insecure PRNG for Session IDs]
**Vulnerability:** The application was using `Math.random()` to generate wishlist session IDs in `src/app/api/wishlist/route.ts`. `Math.random()` is not a cryptographically secure pseudorandom number generator (CSPRNG) and produces predictable outputs. This predictability could allow an attacker to guess active session IDs, potentially leading to session hijacking, wishlist takeover, or unintended data disclosure.
**Learning:** `Math.random()` should never be used for security-sensitive operations, such as generating session identifiers, authentication tokens, or cryptographic keys, even when concatenated with a timestamp. The predictable nature of the algorithm allows for brute-force or state-guessing attacks.
**Prevention:** Always use a cryptographically secure generator like `crypto.randomUUID()` or `crypto.getRandomValues()` provided by the Web Crypto API for generating secure tokens, identifiers, or random data on the server-side.
