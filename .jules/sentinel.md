...
## 2025-03-10 - Critical: Destructive Root-Level API Routes Missing Authorization
**Vulnerability:** Destructive root-level API routes sitting outside the path-based middleware protection for `/admin` and `/api/admin` (such as `/api/categories/[id]`, `/api/books/[slug]`, and `/api/settings/hero`) lacked explicit `verifyAdminSession` checks on mutative methods (PUT, PATCH, DELETE). This allowed any unauthenticated user to modify or delete categories, books, and hero settings.
**Learning:** Endpoints outside of path-based middleware protection must have explicit authorization checks. Middleware matching paths shouldn't be the only line of defense for endpoints not clearly grouped by path prefix.
**Prevention:** Always implement `verifyAdminSession(request)` checks on mutative methods (PUT, PATCH, DELETE) to prevent unauthorized modifications and ensure defense-in-depth, regardless of the route's path prefix.
