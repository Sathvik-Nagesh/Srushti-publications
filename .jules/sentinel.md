...
## 2025-03-20 - High: Brute-Force Vulnerability in Admin Change Password
**Vulnerability:** The `POST /api/admin/change-password` endpoint lacked rate limiting. Although it verified the admin session, it allowed an authenticated attacker to make unlimited attempts to guess the current admin password, making it susceptible to brute-force attacks on the current password validation.
**Learning:** Even if an endpoint requires authentication, sensitive operations like changing passwords (which require verifying the current password) must be rate-limited to prevent brute-force attacks on the current password validation.
**Prevention:** Always apply rate limiting to endpoints handling password verification, password changes, or login attempts, regardless of session-based authentication checks.
