...
## 2025-05-18 - Avoid Security Theater with Web Crypto API
**Vulnerability:** Replacing `Math.random()` with `crypto.randomUUID()` in frontend code (like UI state or non-critical identifiers) can cause application crashes in non-secure contexts (e.g., local network testing over HTTP) where the Web Crypto API is unavailable.
**Learning:** Only apply cryptographic security enhancements (like `crypto.getRandomValues()` or `crypto.randomUUID()`) to backend-generated, security-critical identifiers (like session tokens or order numbers) where the secure context is guaranteed. Applying it to non-critical frontend identifiers is "security theater" and introduces regression risks.
**Prevention:** Restrict the use of Web Crypto API functions to server-side code (API route handlers) or explicitly check for `window.crypto` availability on the frontend if cryptographic randomness is truly required.
