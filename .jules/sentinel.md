...
## 2026-05-01 - [MEDIUM] Insecure Randomness in Wishlist Session IDs\n**Vulnerability:** The  endpoint generated guest session identifiers using `Math.random()`. This function is not cryptographically secure, making session IDs predictable and potentially allowing an attacker to hijack or guess wishlist sessions.\n**Learning:** Backend generation of identifiers or tokens should never rely on `Math.random()`. While it's acceptable for client-side non-critical identifiers (to avoid crashes on non-secure connections), server-side logic must use cryptographic functions.\n**Prevention:** Use `crypto.randomUUID()` (which uses Web Crypto API in Edge/modern environments) or `crypto.getRandomValues()` for generating session IDs, tokens, or order numbers in all backend route handlers.
## 2026-05-01 - [MEDIUM] Insecure Randomness in Wishlist Session IDs
**Vulnerability:** The `/api/wishlist` endpoint generated guest session identifiers using `Math.random()`. This function is not cryptographically secure, making session IDs predictable and potentially allowing an attacker to hijack or guess wishlist sessions.
**Learning:** Backend generation of identifiers or tokens should never rely on `Math.random()`. While it's acceptable for client-side non-critical identifiers (to avoid crashes on non-secure connections), server-side logic must use cryptographic functions.
**Prevention:** Use `crypto.randomUUID()` (which uses Web Crypto API in Edge/modern environments) or `crypto.getRandomValues()` for generating session IDs, tokens, or order numbers in all backend route handlers.
